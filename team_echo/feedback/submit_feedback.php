...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($category_id) || empty($feedback_text)) {
        ...
        $sql = "INSERT INTO feedback (category_id, feedback_text, is_anonymous) VALUES (?, ?, 1)";
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("is", $category_id, $feedback_text);
            if ($stmt->execute()) {
...
@media(max-width:900px) {
    ...
    .left, .right { flex: unset; padding: 30px; }
    .animated-text { text-align: center; border-right: 4px solid white; }
}
</style>
</head>
<body>
